...
void handle_buttons() {
    ...
    if (!k1 && key1_prev && (now - key1_t_us) > kDebounceUs) {
        ...
        if (current_screen == 2) {
            ...
            lb_mode = (lb_mode + 1) % 5;  // cycle LIVE -> F0 -> F1 -> F2 -> F3 -> LIVE
        } else {
            send_rumble(0xC0);
            rumble_active = true;
...
void render_screen_lightbar() {
    ...
    char tag[10];
    if (lb_mode == 0) snprintf(tag, sizeof(tag), "[LIVE]");
    else              snprintf(tag, sizeof(tag), "[FAV%d]", lb_mode - 1);
    draw_text(86, 0, tag);
    ...
    if (bt_is_connected()) {
        ...
        draw_text(0, 48, lb_mode == 0 ? "Tilt = R/G/B" : "Locked to fav");
